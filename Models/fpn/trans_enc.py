...
class MSDeformAttnPixelDecoder(nn.Module):
    def __init__(
        ...
        transformer_in_features = list(transformer_in_features[0])
        ...
        if self.transformer_num_feature_levels > 1:
            ...
            self.input_proj = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(transformer_in_channels[-1], conv_dim, kernel_size=3, stride=1, padding=1),
                    nn.GroupNorm(32, conv_dim),
                )])
    ...
    def forward_features(self, features):
        bac_feature = list(features.values())
        bac_feature = bac_feature[::-1]
        srcs = []
        pos = []
        ...
        for idx, f in enumerate(out):
            multi_scale_features.append(self.output_proj[idx](f))

        return multi_scale_features[0], bac_feature, multi_scale_features
